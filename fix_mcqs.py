...
file_path = 'D:/Internship/Teach-ai/Frontend/src/data/problems.ts'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Replace the "mcqs": [...] array
# Since json.dumps produces double quotes and compact layout, let's format it nicely
new_mcqs_str = '"mcqs": ' + json.dumps(new_mcqs, indent=6)

# We use regex to find all "mcqs": [ ... ] arrays and replace them
# We know they start with "mcqs": [ and end with ]
new_content = re.sub(r'\"mcqs\": \[\s*\{.*?\}\s*\]', new_mcqs_str, content, flags=re.DOTALL)

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(new_content)
    
print('Updated problems.ts with 20 questions for all problems!')
